...
$ff = mysqli_query($conn, "select nombre_cliente,apellido_cliente,id_cliente,documento_cliente
from clientes
where lower(nombre_cliente) like'%$criterio%'") or die("ERROR EN SELECCION EN VENTA: " . mysql_error());
...
while ($productos = mysqli_fetch_array($ff)) {
    ...
    print "{ \"label\" : \"$productos[0] $productos[1]\",
